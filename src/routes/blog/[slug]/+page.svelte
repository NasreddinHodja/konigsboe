...
<div class="mx-auto max-w-3xl px-6 py-16">
	<nav class="mb-8 flex items-center gap-1.5 text-xs text-fg-muted font-mono">
		<a href="/" class="hover:underline underline-offset-2 text-fg-muted">Home</a>
		<ChevronRight class="h-3 w-3" />
		<a href="/blog" class="hover:underline underline-offset-2 text-fg-muted">Writing</a>
		<ChevronRight class="h-3 w-3" />
	...
	<header class="mb-12 pb-8 {data.post.html?.trim() ? 'border-b border-border' : ''}">
		...
		<h1 class="text-4xl font-bold leading-tight font-display">
	...
	{#if toc.length > 0}
		<nav class="mb-12 border border-border p-5 text-sm bg-accent-bg">
			<p class="mb-3 text-xs uppercase tracking-widest text-fg-muted font-mono">
				Contents
			</p>
			<ul class="space-y-1.5">
				{#each toc as item}
					<li style="padding-left:{(item.level - 2) * 1}rem">
						<a href="#{item.id}" class="underline-offset-2 hover:underline text-fg">
							{item.text}
						</a>
					</li>
				{/each}
	...
	{#if data.prev || data.next}
		<nav class="mt-16 flex justify-between gap-4 border-t border-border pt-8">
			{#if data.prev}
				<a href="/blog/{data.prev.slug}" class="group max-w-[45%] text-sm">
					<span class="mb-1 flex items-center gap-1 text-xs text-fg-muted font-mono"><ArrowLeft class="h-3 w-3" /> Older</span>
					<span class="font-medium underline-offset-2 group-hover:underline font-display">
			...
			{/if}
			{#if data.next}
				<a href="/blog/{data.next.slug}" class="group max-w-[45%] text-right text-sm">
					<span class="mb-1 flex items-center justify-end gap-1 text-xs text-fg-muted font-mono">Newer <ArrowRight class="h-3 w-3" /></span>
					<span class="font-medium underline-offset-2 group-hover:underline font-display">
