...
<div class="mx-auto max-w-3xl px-6 py-16">
	<nav class="mb-8 flex items-center gap-1.5 font-mono text-xs text-fg-muted">
		...
		<a href="/pandeiro" class="text-fg-muted underline-offset-2 hover:underline">Pandeiro</a>
		<ChevronRight class="h-3 w-3" />
			...
			<ul class="space-y-1.5">
				{#each toc as item (item.id)}
					<li style="padding-left:{(item.level - 2) * 1}rem">
						<a href="#{item.id}" class="text-fg underline-offset-2 hover:underline">
							{item.text}
						</a>
					</li>
				{/each}
	...
	{#if data.prev || data.next}
		<nav class="mt-16 flex justify-between gap-4 border-t border-border pt-8">
			{#if data.prev}
				<a href="/pandeiro/{data.prev.slug}" class="group max-w-[45%] text-sm">
			...
			{/if}
			{#if data.next}
				<a href="/pandeiro/{data.next.slug}" class="group max-w-[45%] text-right text-sm">
